 ...
 * @param <T>
 */
public class _OrcModel<T extends Entity> extends DDModel<T> implements ArmedModel {
	 ...
	 * @param root
	 */
	public _OrcModel(ModelPart root) {
		this.root = root;
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		// head
//		DD.LOGGER.debug("netHeadYaw -> {}", netHeadYaw);
		...
		// NOTE currently can't enable if holding a weapon because the weapon pivot point is wrong
		// and will render way out of the hand if the body rotates
	...
	// 
	/**
	 * required because of ArmedModel.
	 * this is actually translating to pivot of arm/shoulder joint
	 */
	@Override
	public void translateToHand(HumanoidArm arm, PoseStack poseStack) {
		// flip on y-axis so blade is in right direction.
		// this in turn negatives the x,y,z movements
		poseStack.mulPose(Vector3f.YP.rotationDegrees(180.0F));
		
		// this translates to the arm/should joint/pivot point.
		// NOTE (+) pushes it down
		poseStack.translate((double)(-rightArm.x / 16.0F), ((this.getRightArm().y+20)/16), (double)((rightArm.z)/16.0F) );
				
		// set all the rotations as the same as the arm
		poseStack.mulPose(Vector3f.YP.rotation(-this.getRightArm().yRot));
		poseStack.mulPose(Vector3f.XP.rotation(-this.getRightArm().xRot));
		poseStack.mulPose(Vector3f.ZP.rotation(-this.getRightArm().zRot));		
		
		// face the proper direction. ie rotate sword point upwards towards N
		poseStack.mulPose(Vector3f.XP.rotationDegrees(90F)); // counter-clock
		// add an extra because the arm is bent at the elbow. not a right-angle like a zombie
		poseStack.mulPose(Vector3f.XP.rotationDegrees(25F)); // counter-clock
...
//		rightLeg.render(poseStack, buffer, packedLight, packedOverlay);
	...
	public void resetSwing(T entity, ModelPart body, ModelPart rightArm, ModelPart leftArm) {
		// NOTE setting body.yRot = 0 here negates the effect of the orc body rotating when looking at you.
//		body.yRot = 0;
	...
	public void translateToPivot(ModelPart rightArm, PoseStack poseStack) {
		// flip on y-axis so blade is in right direction.
		// this in turn negatives the x,y,z movements
		poseStack.mulPose(Vector3f.YP.rotationDegrees(180.0F));
		
//		DD.LOGGER.debug("x -> {}, y-> {}, z-> {}", rightArm.x, rightArm.y, rightArm.z);
		poseStack.translate((double)(-rightArm.x / 16.0F), ((this.getRightArm().y+44)/16), (double)((rightArm.z)/16.0F) );
		
		poseStack.mulPose(Vector3f.YP.rotation(rightArm.yRot));
		poseStack.mulPose(Vector3f.XP.rotation(rightArm.xRot));
		poseStack.mulPose(Vector3f.ZP.rotation(rightArm.zRot));
	}
}